...
📚 **更多文档**：[生产部署报告](docs/deployment-report.md) ｜ [客户端接入指南](docs/client-integration-guide.md)（飞花令等项目接入必读）
...
服务启动后，在浏览器直接访问 **`http://localhost:8080`** 即可打开 **Web 实时语音识别控制台**（支持 PC 与手机移动端竖屏录音测试）。开发模式可用默认 Token `default-client-token`；生产部署（见部署报告）需在控制台 ⚙️ 设置中填入管理员分配的 API Key。
